...
export const getStoryByUsername = async(req,res)=>{
  ...
  const story = await Story.find({author:user._id}).populate('viewers','name username profilePicture').populate('author','name username profilePicture');

  return res.status(200).json(story);

  } catch (error) {
    return res.status(500).json({message:'Error while checking story by username !'});
  }
