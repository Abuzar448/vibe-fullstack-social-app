...
const EditProfile = () => {
  ...
  // 1. Optional chaining lagayi hai taaki refresh pe crash na ho
  // 2. 'userName' ko 'username' kiya hai backend model ke hisaab se
  const [frontendImage, setFrontendImage] = useState(
    userData?.profilePicture || dp,
  );
  ...
  const handleImage = (e) => {
    ...
    if (file) {
      ...
      // Purana blob URL saaf karna memory ke liye achha hota hai
  ...
  const handleEditProfile = async () => {
    ...
    try {
      ...
      formdata.append("username", userName); // Backend 'username' dhoond raha hai
      formdata.append("bio", bio);
      formdata.append("profession", profession);
      formdata.append("gender", gender);

      if (backendImage) {
        // Backend router mein 'profilePicture' key hai, 'profileImage' nahi
      ...
      setLoading(false);
      // Naye username par navigate karein
  ...
  return (
    <div className="w-full min-h-[100vh] bg-black flex items-center flex-col gap-[20px] ">
      ...
      <button
        className="px-[10px] w-[60%] max-w-[400px] py-[5px] h-[50px] bg-[white] cursor-pointer rounded-2xl mb-[20px]"
