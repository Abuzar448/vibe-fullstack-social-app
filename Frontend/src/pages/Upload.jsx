...
function Upload() {
  ...
  const uploadPost = async () => {
    try {
      ...
      const result = await axios.post(
        `http://localhost:8080/api/post/upload`,
        formData,
  ...
  const uploadStory = async () => {
    try {
      ...
      formData.append("media", backendMedia);
      const result = await axios.post(
        `http://localhost:8080/api/story/upload`,
        formData,
        { withCredentials: true },
      );
  ...
  const uploadLoop = async () => {
    try {
      ...
      formData.append("media", backendMedia);
      const result = await axios.post(
        `http://localhost:8080/api/reel/upload`,
        formData,
        { withCredentials: true },
      );
  ...
  return (
    <div className="w-full h-[100vh] bg-black flex flex-col items-center ">
      ...
      <div className="w-[90%] max-w-[600px] h-[80px] bg-[white] rounded-full flex justify-around items-center gap-[10px]">
        <div
          className={`${uploadType == "post" ? "bg-black text-white shadow-2xl shadow-black" : ""}  w-[28%] h-[80%] flex justify-center items-center text-[19px] font-semibold cursor-pointer rounded-full`}
        ...
        <div
          className={`${uploadType == "story" ? "bg-black text-white shadow-2xl shadow-black" : ""}  w-[28%] h-[80%] flex justify-center items-center text-[19px] font-semibold cursor-pointer rounded-full`}
        ...
        <div
          className={`${uploadType == "loop" ? "bg-black text-white shadow-2xl shadow-black" : ""}  w-[28%] h-[80%] flex justify-center items-center text-[19px] font-semibold cursor-pointer rounded-full`}
      ...
      {!frontendMedia && (
        ...
        >
          ...
          />
          <IoMdAdd className="text-white cursor-pointer w-[30px] h-[30px]" />
